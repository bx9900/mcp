...
from awslabs.aws_serverless_mcp_server.models import DeployWebAppRequest, BackendConfiguration, FrontendConfiguration
from awslabs.aws_serverless_mcp_server.tools.webapps.deploy_webapp import (
    deploy_webapp, check_dependencies_installed, check_destructive_deployment_change
)
...
class TestDeployWebapp:
    ...
    async def test_deploy_webapp_destructive_change(self):
        ...
        with patch('awslabs.aws_serverless_mcp_server.tools.webapps.deploy_webapp.check_destructive_deployment_change', 
    ...
    async def test_deploy_webapp_missing_dependencies(self):
        ...
        request = DeployWebAppRequest(
            ...
            backend_configuration=BackendConfiguration(
                built_artifacts_path="/tmp/test-project/dist",
                runtime="nodejs18.x",
                port=3000
            )
        )
        ...
        with patch('awslabs.aws_serverless_mcp_server.tools.webapps.deploy_webapp.check_destructive_deployment_change', 
                  return_value=mock_destructive_check), \
             patch('awslabs.aws_serverless_mcp_server.tools.webapps.deploy_webapp.check_dependencies_installed', 
                  return_value=False):
            # Call the function
            result = await deploy_webapp(request)
