...
class TestGetServerlessTemplates:
    ...
    async def test_get_serverless_templates_content_structure(self):
        ...
        if len(templates) > 0:
            ...
            assert template["gitHubLink"].startswith("https://github.com/aws-samples/serverless-patterns")
    ...
    async def test_get_serverless_templates_with_mocked_github(self):
        ...
        mock_readme_response = {
            "content": "IyBBUEkgR2F0ZXdheSArIExhbWJkYSBFeGFtcGxl"  # Base64 encoded "# API Gateway + Lambda Example"
        }

        with patch('awslabs.aws_serverless_mcp_server.tools.guidance.get_serverless_templates.fetch_github_content') as mock_fetch:
    ...
    async def test_get_serverless_templates_caching(self):
        ...
        request1 = GetServerlessTemplatesRequest(
            template_type="API",
            runtime="python3.9"
        )
        
        request2 = GetServerlessTemplatesRequest(
            template_type="Lambda",
            runtime="nodejs18.x"
        )
    ...
    async def test_get_serverless_templates_limit(self):
        ...
        mock_tree_response = {
            "tree": [
                {"path": f"lambda-example-{i}", "type": "tree"} for i in range(10)
            ]
        }
