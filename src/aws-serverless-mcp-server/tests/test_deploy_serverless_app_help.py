...
class TestDeployServerlessAppHelp:
    ...
    async def test_deploy_serverless_app_help_backend(self):
        ...
        all_prompts = " ".join([step["prompt"] for step in content])
        assert "api gateway" in all_prompts.lower() or "lambda function urls" in all_prompts.lower()
    ...
    async def test_deploy_serverless_app_help_content_consistency(self):
        ...
        assert all(count == step_counts[0] for count in step_counts), "All application types should have the same number of steps"
    ...
    async def test_deploy_serverless_app_help_step_order(self):
        ...
        assert any(keyword in step_prompts[0] for keyword in ["init", "generate", "handler", "structure"])
