...
async def deploy_application(request: DeployWebAppRequest) -> Dict[str, Any]:
    ...
    await initialize_deployment_status(
        project_name, 
        deployment_type,
        framework,
        request.region
    ...
    try:
        ...
        if (deployment_type in ['backend', 'fullstack'] and 
                request.backend_configuration):
            ...
            if not os.path.isabs(backend_config.built_artifacts_path):
                backend_config.built_artifacts_path = os.path.join(
                    project_root,
                    backend_config.built_artifacts_path
                )
        ...
        if (deployment_type in ['frontend', 'fullstack'] and 
                request.frontend_configuration):
            ...
            if not os.path.isabs(frontend_config.built_assets_path):
                frontend_config.built_assets_path = os.path.join(
                    project_root,
                    frontend_config.built_assets_path
                )
        ...
        if (deployment_type in ['backend', 'fullstack'] and 
            ...
            if backend_config.startup_script:
                ...
                if os.path.isabs(backend_config.startup_script):
                    raise Exception("Startup script must be relative to built_artifacts_path, not an absolute path. Please provide a path relative to the built_artifacts_path directory.")
                
                # Resolve the full path to the built_artifacts_path
                full_artifacts_path = os.path.join(
                    project_root,
                    backend_config.built_artifacts_path
                ) if not os.path.isabs(backend_config.built_artifacts_path) else backend_config.built_artifacts_path
            ...
            # Generate a startup script if requested
                ...
                try:
                    ...
                    raise Exception(f"Failed to generate startup script: {str(e)}. Please check that your entry point file exists in the built artifacts directory and the path is correct.")
                except Exception as e:
                    raise e
            # Neither startup script nor generate_startup_script+entry_point provided
            elif not backend_config.startup_script:
                raise Exception("No startup script provided or generated. Please either provide a startup_script or set generate_startup_script=true with an entry_point.")
        ...
        if (deployment_type in ['frontend', 'fullstack'] and 
                request.frontend_configuration.built_assets_path):
