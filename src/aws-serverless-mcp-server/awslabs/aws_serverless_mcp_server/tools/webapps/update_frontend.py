...
async def get_all_files(dir_path: str, array_of_files: Optional[List[str]] = None, base_path: Optional[str] = None) -> List[str]:
    """
...
async def sync_directory_to_s3(
    s3_client: Any,
    directory_path: str,
    bucket_name: str
) -> None:
    ...
    try:
        ...
        if not os.path.exists(built_assets_path):
            return {
                ...
                'content': [{'type': 'text', 'text': f"Error: Built assets path not found: {built_assets_path}"}]
            }
        ...
        try:
            ...
            if not describe_stacks_result.get('Stacks'):
                return {
                    ...
                    'content': [{'type': 'text', 'text': f"Error: CloudFormation stack {stack_name} not found. Please deploy the application first using the deploy tool."}]
                }
            
            # Extract the S3 bucket name from stack outputs
            outputs = describe_stacks_result['Stacks'][0].get('Outputs', [])
            bucket_output = next((output for output in outputs if output.get('OutputKey') == 'WebsiteBucket'), None)
            ...
            if not bucket_output or not bucket_output.get('OutputValue'):
                return {
                    ...
                    'content': [{'type': 'text', 'text': f"Error: Could not find WebsiteBucket output in CloudFormation stack {stack_name}. This suggests the stack was not deployed as a frontend or fullstack application."}]
                }
            ...
            logger.info(f"Uploading frontend assets from {built_assets_path} to bucket {bucket_name}")
            ...
            cloudfront_output = next((output for output in outputs if 
                output.get('OutputKey') in ['CloudFrontDistribution', 'CloudFrontDomain', 'CloudFrontDistributionId', 'CloudFrontURL']
            ), None)
            ...
            if cloudfront_output and cloudfront_output.get('OutputValue'):
                ...
                if distribution_id.startswith('http'):
                    distribution_id_output = next((output for output in outputs if 
                        output.get('OutputKey') == 'CloudFrontDistributionId'
                    ), None)
                    
                    if distribution_id_output and distribution_id_output.get('OutputValue'):
                        distribution_id = distribution_id_output['OutputValue']
                    else:
                        logger.warning("Found CloudFront URL but no distribution ID, skipping invalidation")
                        return {
                            'status': 'success',
                            'message': f"Frontend assets updated successfully for {project_name}, but couldn't create CloudFront invalidation",
                            'content': [
                                {'type': 'text', 'text': f"Frontend assets for {project_name} have been successfully updated."},
            ...
            return {
                ...
                'content': [
                    {'type': 'text', 'text': f"Frontend assets for {project_name} have been successfully updated."},
                    {'type': 'text', 'text': f"Assets were uploaded to S3 bucket: {bucket_name}"},
                    {'type': 'text', 'text': "CloudFront cache invalidation has been initiated and may take a few minutes to complete."} if cloudfront_output else
            ...
            if 'does not exist' in str(e):
                return {
                    ...
                    'content': [{'type': 'text', 'text': f"Error: CloudFormation stack {stack_name} does not exist. Please deploy the application first using the deploy tool."}]
                }
            ...
            return {
                ...
                'content': [{'type': 'text', 'text': f"Error: Failed to update frontend assets: {str(e)}"}]
            }
