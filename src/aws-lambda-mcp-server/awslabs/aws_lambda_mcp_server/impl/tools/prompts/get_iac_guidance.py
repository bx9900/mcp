...
async def get_iac_guidance(request: GetIaCGuidanceRequest) -> Dict[str, Any]:
    """
    Get guidance on Infrastructure as Code (IaC) tools for AWS.
    
    Args:
        request: GetIaCGuidanceRequest object containing request parameters
...
Resources:
  MyLambdaFunction:
    ...
    comparison_table = ComparisonTable(
        ...
        rows=[
            ...
            },
            {
                ...
                "cells": ["High (serverless-focused)", "High (programmable)", "Low (raw resources)"]
            },
            {
                "tool": "Local Testing",
                "cells": ["Yes (sam local)", "Limited", "No"]
            },
            {
                "tool": "Resource Coverage",
                "cells": ["Serverless-focused but supports all AWS resources", "All AWS resources", "All AWS resources"]
            }
        ]
    )
    ...
    if request.iac_tool:
        if request.iac_tool == "CloudFormation":
            tool_specific_guidance = ToolSpecificGuidance(
                ...
                common_commands=[
                    {"command": "aws cloudformation validate-template", "description": "Validate a template"},
                    {"command": "aws cloudformation create-stack", "description": "Create a new stack"},
                    {"command": "aws cloudformation update-stack", "description": "Update an existing stack"},
                    {"command": "aws cloudformation describe-stacks", "description": "Get information about stacks"},
                    {"command": "aws cloudformation delete-stack", "description": "Delete a stack"}
                ]
            )
        elif request.iac_tool == "SAM":
            tool_specific_guidance = ToolSpecificGuidance(
                ...
                common_commands=[
                    {"command": "sam init", "description": "Initialize a new SAM application", "mcpTool": "sam_init"},
                    {"command": "sam build", "description": "Build your application", "mcpTool": "sam_build"},
            ...
            tool_specific_guidance = ToolSpecificGuidance(
                ...
                common_commands=[
                    ...
                    {"command": "cdk synth", "description": "Synthesize CloudFormation template"},
                    {"command": "cdk diff", "description": "Compare deployed stack with current state"},
