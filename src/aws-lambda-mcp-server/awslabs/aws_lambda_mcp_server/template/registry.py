...
class Template:
    ...
    def __init__(self, name: str, path: str, type_: DeploymentTypes, framework: Optional[str] = None):
        self.name = name
        self.path = path
        self.type = type_
...
async def get_template_for_deployment(deployment_type: DeploymentTypes, framework: Optional[str] = None) -> Template:
    """
    ...
    logger.debug(f"Looking for template with deployment type: {deployment_type}, framework: {framework or 'none'}")
    ...
    if framework:
        search_paths.append(os.path.join(templates_path, f"{deployment_type}-{framework}.hbs"))
        search_paths.append(os.path.join(templates_path, f"{deployment_type}-{framework}.yaml"))
        search_paths.append(os.path.join(templates_path, f"{deployment_type}-{framework}.yml"))

    # 2. Default template for this deployment type
    search_paths.append(os.path.join(templates_path, f"{deployment_type}-default.hbs"))
    search_paths.append(os.path.join(templates_path, f"{deployment_type}-default.yaml"))
    search_paths.append(os.path.join(templates_path, f"{deployment_type}-default.yml"))

    # 3. Generic template for this deployment type
    search_paths.append(os.path.join(templates_path, f"{deployment_type}.hbs"))
    search_paths.append(os.path.join(templates_path, f"{deployment_type}.yaml"))
    search_paths.append(os.path.join(templates_path, f"{deployment_type}.yml"))
...
async def discover_templates() -> List[Template]:
    ...
    try:
        ...
        for file in files:
            ...
            logger.debug(f"Added template: {name}, type: {type_}, framework: {framework or 'none'}")
...
async def list_templates() -> List[Template]:
    ...
    Returns:
        List[Template]: List of available templates
    """
    return await discover_templates()


async def get_template_info(template_name: str) -> Template:
    """
